...
const initial = {
  theme: 'dark' as 'dark' | 'purple' | 'light',
  animations: true,
  compact: false,
...
export default function Interface({ addToast }: Props) {
  ...
  const BtnGroup = ({ value, options, onChange }: { value: string; options: {label:string;val:string}[]; onChange: (v: string)=>void }) => (
    <div style={{ display:'flex', gap:4 }}>
      {options.map(o => (
        <button key={o.val} className={`btn btn-sm ${value===o.val ? 'btn-primary' : 'btn-secondary'}`} onClick={() => onChange(o.val)}>
          {o.label}
        </button>
      ))}
    </div>
    ...
    <div className="page-content">
      ...
      <div className="page-header" style={{ marginTop: 20 }}>
        <div>
          ...
          <div className="page-subtitle">Customize your dashboard experience</div>
        </div>
      </div>
      ...
      <div className="card">
        <div className="section" style={{ paddingTop:0 }}>
          <div className="section-header">
            ...
            <BtnGroup value={state.theme} options={[{label:'Dark',val:'dark'},{label:'SyncInk Purple',val:'purple'},{label:'Light',val:'light'}]} onChange={v => set('theme', v as 'dark'|'purple'|'light')} />
          </div>
        </div>
        ...
        <div className="section">
          <div className="section-header">
            ...
            <BtnGroup value={state.sidebar} options={[{label:'Expanded',val:'expanded'},{label:'Collapsed',val:'collapsed'}]} onChange={v => set('sidebar', v as 'expanded'|'collapsed')} />
          </div>
        </div>

        <div className="section" style={{ paddingBottom:0, borderBottom:'none' }}>
          <div style={{ fontWeight:600, fontSize:14, color:'var(--text-primary)', marginBottom:14 }}>Notification Settings</div>
          {[
            ...
            <div key={item.key} className="section" style={{ borderBottom: i===arr.length-1?'none':undefined, paddingTop: i===0?0:undefined }}>
              <div className="section-header">
                <div>
